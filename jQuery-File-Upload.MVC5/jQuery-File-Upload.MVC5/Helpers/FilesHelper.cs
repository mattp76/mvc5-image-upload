    ...
    public class FilesHelper
    {
        ...
        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                var x = 1;
                while (File.Exists(Path.Combine(pathOnServer, Path.GetFileName(fileName))))
                {
                    fileName = nameWithoutExtension.Trim() + " (" + x + ")" + extension;
                    x++;
                }

                var fullPath = Path.Combine(pathOnServer, Path.GetFileName(fileName));
                ...
                if (imageArray.Length != 0)
                {
                    ...
                    else
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream(System.IO.File.ReadAllBytes(fullPath)))
                        {
                            ...
                            var y = 1;
                            while (File.Exists(Path.Combine(ThumbfullPath, Path.GetFileName(fileThumb))))
                            {
                                fileThumb = thumbNameWithoutExtension.Trim() + " (" + y + ")" + ".jpg";
                                y++;
                            }
